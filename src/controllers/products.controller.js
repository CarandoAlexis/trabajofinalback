...
export const addProduct = async (req, res, next) => {
  try {
    ...
    logger.info(`Owner que agrego el producto: ${owner}`);
...
export const editProduct = async (req, res) => {
  try {
    ...
    const { title, description, price, code, category, owner } = req.body;
    ...
    if (user.role === "admin" || user.email === product.owner) {
      const updatedProduct = await productService.editProduct(productId, {
        title,
        description,
        price,
        code,
    ...
    res
      .status(500)
      .json({ status: "error", message: "Error al editar el producto" });
  }
};
