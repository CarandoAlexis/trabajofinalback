...
export const getAllUsers = async (req, res) => {
  try {
    const users = await UserModel.find({ role: { $ne: "admin" } }, 'first_name last_name email role');
...
export const deleteInactiveUsers = async (req, res) => {
  try {
    ...
    inactiveUsers.forEach(async (user) => {
      ...
      const text = 'Tu cuenta ha sido eliminada por inactividad. Si deseas recuperarla, contáctanos.';
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (newRole !== 'usuario' && newRole !== 'premium') {
      return res.status(400).json({ message: 'Rol no válido. Debe ser "usuario" o "premium".' });
    }

    user.role = newRole;
    await user.save();

    return res.json({ message: `Rol de usuario actualizado a ${newRole}`, updatedUser: user });
  } catch (error) {
